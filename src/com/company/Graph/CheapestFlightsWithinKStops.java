...
13
    ...
    public int findCheapestPrice(int n, int[][] flights, int src, int dst, int K) {
        ...
        dfs(src, dst, K, map, set, 0);
        return result == Integer.MAX_VALUE ? -1 : result;
    }

    private void dfs(int src, int dst, int k, Map<Integer, List<List<Integer>>> map, Set<Integer> visited, int costYet) {
        System.out.println("*****************************************");
        System.out.println("src:" + src);
        System.out.println("dst:" + dst);
        System.out.println("k:" + k);
        System.out.println(visited);
        System.out.println("CostYet:" + costYet);
        if (src == dst) {
            result = Math.min(result, costYet);
            return;
        }
        if (visited.contains(src)) return;
        if (result < costYet) return;
        visited.add(src);
        if (map.containsKey(src)) {
            for (List<Integer> list : map.get(src)) {
                if (k >= 0) {
                    dfs(list.get(0), dst, k - 1, map, visited, costYet + list.get(1));
                }
            }
        }
        visited.remove(src);
